...
export default function Workshops() {
  ...
  // Helper to determine if a workshop is upcoming or past
  const isWorkshopUpcoming = (startDate: string, endDate?: string) => {
    const start = new Date(startDate);
    const end = endDate ? new Date(endDate) : start;
    return end >= today; // Upcoming if the end date hasn't passed yet
  };

  const upcomingWorkshops = workshops.filter(w =>
    isWorkshopUpcoming(w.startDate, w.endDate)
  );
      ...
      <main className="flex-1 px-8 sm:px-20 py-12 flex flex-col relative z-10">
        ...
        >
          ...
          <p className="text-xl text-gray-400 mb-10 max-w-3xl mx-auto">
            Expand your skills with our interactive workshops led by industry
            experts and faculty members. From beginner to advanced levels, we've
        ...
        >
          <div className="flex items-center mb-8">
            <h2 className="text-3xl font-bold font-mono text-white mr-4">
              Upcoming Workshops
            </h2>
