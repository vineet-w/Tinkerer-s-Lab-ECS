...
// ✅ Format date range like: "21 Jul 2025 → 25 Jul 2025"
...
export default function WorkshopCard({ workshop, isPast }: WorkshopCardProps) {
  ...
  return (
    <motion.div
      className={`group relative bg-[#1A1A1A]/50 backdrop-blur-lg rounded-2xl border overflow-hidden ${
        isPast ? "border-[#9F70FD]/10" : "border-[#9F70FD]/30"
      }`}
    ...
    >
      ...
      <div className="p-6">
          ...
          {workshop.tags.map((tag, index) => (
            <span
              ...
              className={`px-3 py-1 text-xs font-mono rounded-full ${
                isPast
                  ? "bg-[#9F70FD]/10 text-[#9F70FD]"
                  : "bg-[#9F70FD]/20 text-[#00F5D4]"
              }`}
            >
              {tag}
            </span>
        ...
        <p className={`mb-4 ${isPast ? "text-gray-500" : "text-gray-400"}`}>
        ...
        <div className="space-y-3 mb-6">
          <div className="flex items-center text-gray-300">
            <FiCalendar
              className={`mr-3 ${isPast ? "text-[#9F70FD]" : "text-[#00F5D4]"}`}
            />
        ...
        {isPast ? (
          ...
          <motion.a
            ...
            className="w-full block text-center px-6 py-3 bg-gradient-to-r from-[#9F70FD] to-[#00F5D4] text-white rounded-lg font-bold font-mono"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            Register Now <FiArrowRight className="inline ml-2" />
          </motion.a>
        ) : (
          <button
            className="w-full px-6 py-3 bg-[#9F70FD]/10 text-[#9F70FD] rounded-lg font-bold font-mono flex items-center justify-center cursor-not-allowed"
